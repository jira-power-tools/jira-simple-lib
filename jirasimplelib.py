...
def update_story_assignee(jira, story_key, new_assignee):
    try:
        ...
        payload = {
            "fields": {"assignee": {"name": new_assignee} if new_assignee else None}
        }
        ...
        response = requests.put(
            ...
            json=payload,
            auth=(
                jira._session.auth[0],
                jira._session.auth[1],
        ...
        if response.status_code == 204:
            ...
            print(
                f"Failed to update story assignee. Key: {story_key}. Status code: {response.status_code}"
            )
            return None
...
def assign_issue(jira, issue_key, assignee_username):
    try:
        ...
        logger.info(
            f"Issue {issue_key} assigned to user {assignee_username} successfully."
        )
    except Exception as e:
        logger.error(
            f"Error assigning issue {issue_key} to user {assignee_username}: {e}"
        )
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        dest="issue_key",
        type=str,
        help="Key of the Jira issue",
    )
    parser.add_argument(
        "--assign-issue",
        nargs=2,
        metavar=("issue_key", "assignee_username"),
    ...
    parser.add_argument(
        "--update-project",
        nargs=3,
        metavar=("\tproject_key", "new_name", "new_key"),
    ...
    parser.add_argument(
        "--delete-all-projects", help="Delete all projects", action="store_true"
    )
    parser.add_argument(
        "--delete-project",
        metavar="\tproject_key",
    ...
    parser.add_argument(
        "--create-epic",
        nargs=3,
        metavar=("\tproject_key", "epic_name", "epic_summary"),
    ...
    parser.add_argument(
        "--update-epic",
        nargs=3,
        metavar=("\tepic_key", "new_summary", "new_description"),
    ...
    parser.add_argument(
        ...
        dest="board_id",
        help="ID of the board for which to retrieve sprints",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--start-sprint",
        nargs=4,
        metavar=("\tsprint_id", "new_summary", "start_date", "end_date"),
        help="\nStart a sprint",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--complete-sprint",
        nargs=3,
        metavar=("\tsprint_id", "start_date", "end_date"),
        help="\nComplete a sprint",
    )
    parser.add_argument(
        "--update-sprint-summary",
        nargs=5,
        metavar=("sprint_id", "new_summary", "sprint_state", "start_date", "end_date"),
        help="Update sprint summary",
    )
    parser.add_argument(
        "--sprint-report",
        nargs=2,
        metavar=("\tsprint_id", "project_key"),
        help="\nGenerate sprint report",
    )
    parser.add_argument(
        "--delete-sprint", nargs=1, metavar=("\tsprint_id"), help="\nDelete a sprint"
    )
    parser.add_argument(
        "--delete-all-sprints", action="store_true", help="\nDelete all sprints"
    )
    parser.add_argument(
        "--create-board",
        nargs=3,
        metavar=("\tproject_key", "project_lead", "user_email"),
        help="\hCreate a new board",
    )
    parser.add_argument(
        "--get-board-id",
        nargs=1,
...
def main():
    ...
    if args.update_project:
        ...
        if update_jira_project(jira, project_key, new_name, new_key):
            logging.info(
                f"Project '{project_key}' updated successfully with new name '{new_name}' and key '{new_key}'."
            )
        else:
            logging.error(
                f"Failed to update project '{project_key}' with new name '{new_name}' and key '{new_key}'."
            )
    ...
    if args.get_stories:
        ...
        if stories:
            render_tui(
                stories, fetching_data=False
            )  # Not fetching data when getting stories
    else:
    ...
    if args.create_sprint:
        sprint_id = create_sprint(
            args.url, args.username, args.api_token, args.board_id, *args.create_sprint
        )
        if sprint_id:
    ...
    if args.move_issues_to_sprint:
        project_key, start_issue_key, end_issue_key, target_sprint_id = (
            args.move_issues_to_sprint
        )
        move_issues_to_sprint_tui(
            jira, project_key, start_issue_key, end_issue_key, target_sprint_id
        )
