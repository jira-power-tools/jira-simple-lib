...
class TestInitialize(unittest.TestCase):
    ...
    def test_initialize_with_existing_config(
        self, mock_set_env_vars, mock_load_credentials
    ):
        ...
        mock_set_env_vars.assert_called_once_with(
            {"jira_url": "example.com", "user": "user", "api_token": "token"}
        )
    ...
    def test_initialize_without_existing_config(
        self,
        mock_input,
        mock_set_env_vars,
        mock_save_to_config,
        ...
        mock_save_to_config.assert_called_once_with(
            config_file, "example.com", "user", "token"
        )
        mock_set_env_vars.assert_called_once_with(
            {"jira_url": "example.com", "user": "user", "api_token": "token"}
        )
...
class TestCreateJiraConnection(unittest.TestCase):
    ...
    def test_create_jira_connection_success(self, mock_logging, mock_jira):
        ...
        # Call the function being tested
        jira_connection = create_jira_connection(
            "https://example.com", "username", "api_token"
        )
    ...
    def test_create_jira_connection_missing_inputs(self, mock_logging):
        ...
        mock_logging.error.assert_called_once_with(
            "Missing username, API token, or Jira URL"
        )
    ...
    def test_create_jira_connection_error(self, mock_logging, mock_jira):
        ...
        # Check if the logging was called with the correct message
        mock_logging.error.assert_called_once_with(
            "Error creating Jira connection: Connection error"
        )
...
class TestCreateJiraProject(unittest.TestCase):
    def test_create_project_success(self):
        ...
        jira_mock.create_project.return_value = {
            "key": "PROJECT",
            "name": "Test Project",
        }
...
class TestUpdateJiraProject(unittest.TestCase):
    ...
    def test_update_jira_project_no_jira_connection(self, mock_logging):
        ...
        mock_logging.error.assert_called_once_with(
            "Failed to update project: Jira connection not established."
        )
    ...
    def test_update_jira_project_no_project_key(self, mock_logging):
        ...
        # Check if logging error was called
        mock_logging.error.assert_called_once_with(
            "Failed to update project: Project key not provided."
        )
    ...
    def test_update_jira_project_project_not_found(self, mock_logging):
        ...
        # Check if logging error was called
        mock_logging.error.assert_called_once_with(
            "Project with key 'PROJ' does not exist."
        )
...
class TestListStoriesForProject(unittest.TestCase):
    ...
    def test_list_stories_for_project_valid(self):
        ...
        self.mock_jira.search_issues.assert_called_once_with(
            "project = PROJ AND issuetype in (Bug, Task, Story)"
        )
...
class TestCreateStory(unittest.TestCase):
    ...
    def test_create_story_success(self, mock_logging):
        ...
        mock_logging.info.assert_called_once_with(
            "Story created successfully. Story Key: PROJ-123"
        )
    ...
    def test_create_story_missing_inputs(self, mock_logging):
        ...
        mock_logging.error.assert_called_with(
            "Failed to create story: Jira connection not established."
        )
        self.assertEqual(
            mock_logging.error.call_count, 1
        )  # Make sure only one error was logged
...
class TestUpdateStoryStatus(unittest.TestCase):
    ...
    def test_update_story_status_success(self):
        ...
        self.jira_mock.transition_issue.assert_called_once_with(
            self.jira_mock.issue.return_value, "1"
        )
...
class TestUpdateStorySummary(unittest.TestCase):
    ...
    def test_update_story_summary_success(self, mock_logging):
        ...
        mock_logging.info.assert_called_with(
            "Story summary updated successfully. Key: PROJ-123"
        )
    ...
    def test_update_story_summary_no_issue_key(self, mock_logging):
        ...
        mock_logging.error.assert_called_with(
            "Failed to update story summary: Story key not provided."
        )
    ...
    def test_update_story_summary_no_new_summary(self, mock_logging):
        ...
        mock_logging.error.assert_called_with(
            "Failed to update story summary: New summary not provided."
        )
    ...
    def test_update_story_summary_general_exception(self, mock_logging):
        ...
        with self.assertRaises(Exception):
            update_story_summary(jira_mock, "PROJ-123", "New summary")
        mock_logging.error.assert_called_with(
            "Unexpected error updating story summary: General error"
        )
...
class TestUpdateStoryDescription(unittest.TestCase):
    ...
    def test_update_story_description_success(self, mock_logging):
        ...
        # Verify logging
        mock_logging.info.assert_called_with(
            f"Story description updated successfully. Key: {issue_key}"
        )
    ...
    def test_update_story_description_no_issue_key(self, mock_logging):
        ...
        mock_logging.error.assert_called_with(
            "Failed to update story description: Story key not provided."
        )
    ...
    def test_update_story_description_unexpected_exception(self, mock_logging):
        ...
        with self.assertRaises(Exception):
            update_story_description(mock_jira, "PROJ-123", "Updated description")
        mock_logging.error.assert_called_with(
            "Unexpected error updating story description: Unexpected error"
        )
...
class TestUpdateStoryAssignee(unittest.TestCase):
    ...
    def test_update_story_assignee_success(self, mock_put, mock_get_user_account_id):
        ...
        expected_url = (
            "https://jirasimplelib.atlassian.net/rest/api/2/issue/PROJ-123/assignee"
        )
        expected_headers = {
    ...
    @patch("jsl.get_user_account_id")
    @patch("requests.put")
    def test_update_story_assignee_no_issue_key(
        self, mock_put, mock_get_user_account_id
    ):
        jira_mock = MagicMock()
        with self.assertRaises(ValueError):
    ...
    @patch("jsl.get_user_account_id")
    @patch("requests.put")
    def test_update_story_assignee_no_username(
        self, mock_put, mock_get_user_account_id
    ):
        jira_mock = MagicMock()
        with self.assertRaises(ValueError):
    ...
    @patch("jsl.get_user_account_id")
    @patch("requests.put")
    def test_update_story_assignee_invalid_user(
        self, mock_put, mock_get_user_account_id
    ):
        jira_mock = MagicMock()
        jira_mock.issue.return_value = MagicMock()
    ...
    @patch("jsl.get_user_account_id")
    @patch("requests.put")
    def test_update_story_assignee_request_exception(
        self, mock_put, mock_get_user_account_id
    ):
        jira_mock = MagicMock()
        jira_mock.issue.return_value = MagicMock()
...
class TestGetAssignee(unittest.TestCase):
    ...
    def test_get_assignee_with_assignee(self, mock_logging):
        ...
        mock_logging.info.assert_called_once_with(
            "The assignee of issue PROJ-123 is: John Doe"
        )
    ...
    def test_get_assignee_without_assignee(self, mock_logging):
        ...
        # Check if the logging was called with the correct info message
        mock_logging.info.assert_called_once_with(
            "Issue PROJ-123 is currently unassigned."
        )
...
class TestDeleteStory(unittest.TestCase):
    ...
    def test_delete_story_success_auto_confirm(self, mock_logging):
        ...
        mock_logging.info.assert_called_once_with(
            "Story deleted successfully. Key: PROJ-123"
        )
        self.assertTrue(result)
    ...
    def test_delete_story_success_manual_confirm(self, mock_input, mock_logging):
        ...
        mock_input.assert_called_once_with(
            "Do you really want to delete the story with key PROJ-123? [y/n]: "
        )
        issue_mock.delete.assert_called_once()
        mock_logging.info.assert_called_once_with(
            "Story deleted successfully. Key: PROJ-123"
    ...
    def test_delete_story_cancel_manual_confirm(self, mock_input, mock_logging):
        ...
        mock_logging.info.assert_called_once_with(
            "Story deletion cancelled by user. Key: PROJ-123"
        )
        self.assertFalse(result)
    ...
    def test_delete_story_issue_not_found(self, mock_logging):
        ...
        jira_mock.issue.assert_called_once_with("PROJ-123")
        mock_logging.error.assert_called_once_with(
            "Story with key PROJ-123 does not exist."
        )
        self.assertFalse(result)
    ...
    def test_delete_story_permission_denied(self, mock_logging):
        jira_mock = MagicMock()
        jira_mock.issue.side_effect = JIRAError(
...
class TestAddComment(unittest.TestCase):
    ...
    def test_add_comment_issue_not_found(self, mock_logging):
        ...
        mock_logging.error.assert_called_once_with(
            "Issue with key PROJ-123 does not exist."
        )
...
class TestCreateEpic(unittest.TestCase):
    ...
    def test_create_epic_success(self, mock_logging):
        ...
        jira_mock.create_issue.assert_called_once_with(
            project="PROJ", summary="Epic Summary", issuetype={"name": "Epic"}
        )
        mock_logging.info.assert_called_once_with(
            "Epic created successfully. Epic Key: EPIC-123"
        )
...
class TestListEpics(unittest.TestCase):
    ...
    def test_list_epics_exception(self, mock_logging):
        ...
        mock_logging.error.assert_called_once_with(
            "Error listing epics: Unexpected error"
        )
...
class TestUpdateEpic(unittest.TestCase):
    ...
    def test_update_epic_success(self, mock_logging):
        ...
        mock_logging.info.assert_called_once_with(
            "Epic updated successfully. Key: EPIC-123"
        )
...
class TestAddIssuesToEpic(unittest.TestCase):
    ...
    def test_add_issues_to_epic_epic_not_found(self, mock_logging):
        ...
        mock_logging.error.assert_called_once_with(
            "Epic with key EPIC-123 does not exist."
        )
        self.assertFalse(result)
    ...
    def test_add_issues_to_epic_issue_not_found(self, mock_logging):
        ...
        mock_logging.error.assert_called_once_with(
            "Issue with key ISSUE-1 does not exist."
        )
        self.assertFalse(result)
...
class TestUnlinkStoryFromEpic(unittest.TestCase):
    ...
    def test_unlink_story_from_epic_success(self, mock_logging):
        ...
        mock_logging.info.assert_called_once_with(
            "Story STORY-1 unlinked from its Epic"
        )
        self.assertTrue(result)
...
class TestDeleteEpic(unittest.TestCase):
    ...
    def test_delete_epic_with_auto_confirm(self, mock_input, mock_logging):
        ...
        mock_logging.info.assert_called_once_with(
            "Epic deleted successfully. Key: EPIC-123"
        )
        self.assertTrue(result)
...
class TestListSprints(unittest.TestCase):
    ...
    def test_list_sprints_error(self, mock_logging):
        ...
        jira_mock.sprints.assert_called_once_with(board_id)
        mock_logging.error.assert_called_once_with(
            "Error retrieving sprints for board: Connection error"
        )
        self.assertIsNone(result)
...
class TestStartSprint(unittest.TestCase):
    @patch("jsl.logging")
    def test_start_sprint_already_closed(self, mock_logging):
        jira_mock = MagicMock()
        sprint_id = 2001
        new_summary = "Updated Sprint Summary"
        start_date = "2024-07-10"
        end_date = "2024-07-20"

        sprint_mock = MagicMock()
        sprint_mock.state = "CLOSED"
        jira_mock.sprint.return_value = sprint_mock

        result = start_sprint(jira_mock, sprint_id, new_summary, start_date, end_date)

        jira_mock.sprint.assert_called_once_with(sprint_id)
        mock_logging.warning.assert_called_once_with(
            f"Sprint {sprint_id} is already closed. You cannot restart it."
        )
        self.assertEqual(result, "Sprint is closed")

    @patch("jsl.logging")
    def test_start_sprint_already_active(self, mock_logging):
        jira_mock = MagicMock()
        sprint_id = 2001
        new_summary = "Updated Sprint Summary"
        start_date = "2024-07-10"
        ...
        jira_mock.sprint.assert_called_once_with(sprint_id)
        mock_logging.warning.assert_called_once_with(
            f"Sprint {sprint_id} is already active. No action taken."
        )
        self.assertEqual(result, sprint_mock)


class TestListStoriesInSprint(unittest.TestCase):
    @patch("jsl.logging")
    def test_list_stories_success(self, mock_logging):
        jira_mock = MagicMock()
        sprint_id = 1001
        issue1 = MagicMock(key="PROJ-101")
        issue2 = MagicMock(key="PROJ-102")
        jira_mock.search_issues.return_value = [issue1, issue2]

        result = list_stories_in_sprint(jira_mock, sprint_id)

        jira_mock.search_issues.assert_called_once_with(
            f"sprint = {sprint_id} AND issuetype = Task"
        )
        self.assertEqual(result, ["PROJ-101", "PROJ-102"])

    @patch("jsl.logging")
    def test_list_stories_print_info(self, mock_logging):
        jira_mock = MagicMock()
        sprint_id = 1001
        issue1 = MagicMock(key="PROJ-101")
        issue2 = MagicMock(key="PROJ-102")
        ...
        result = list_stories_in_sprint(jira_mock, sprint_id, print_info=True)

        jira_mock.search_issues.assert_called_once_with(
            f"sprint = {sprint_id} AND issuetype = Task"
        )
        self.assertEqual(result, ["PROJ-101", "PROJ-102"])
        mock_logging.info.assert_any_call(f"Retrieved 2 stories in sprint {sprint_id}")
        mock_logging.info.assert_any_call(f"Story Key: PROJ-101")
        mock_logging.info.assert_any_call(f"Story Key: PROJ-102")


class TestCompleteStoriesInSprint(unittest.TestCase):
    @patch("jsl.list_stories_in_sprint")
    @patch("jsl.update_story_status")
    @patch("jsl.logging")
    def test_complete_stories_success(
        self, mock_logging, mock_update_story_status, mock_list_stories_in_sprint
    ):
        jira_mock = MagicMock()
        sprint_id = 1001
        issue_keys = ["PROJ-101", "PROJ-102"]
        mock_list_stories_in_sprint.return_value = issue_keys
        mock_update_story_status.return_value = True

        result = complete_stories_in_sprint(jira_mock, sprint_id)

        mock_list_stories_in_sprint.assert_called_once_with(jira_mock, sprint_id, False)
        self.assertTrue(result)
        mock_logging.info.assert_called_once_with(
            f"All stories in sprint {sprint_id} marked as completed."
        )

    @patch("jsl.list_stories_in_sprint")
    @patch("jsl.update_story_status")
    @patch("jsl.logging")
    def test_complete_stories_no_stories(
        self, mock_logging, mock_update_story_status, mock_list_stories_in_sprint
    ):
        jira_mock = MagicMock()
        sprint_id = 1001
        mock_list_stories_in_sprint.return_value = None

        result = complete_stories_in_sprint(jira_mock, sprint_id)

        mock_list_stories_in_sprint.assert_called_once_with(jira_mock, sprint_id, False)
        self.assertFalse(result)
        mock_logging.error.assert_called_once_with("No stories found in the sprint.")

    @patch("jsl.list_stories_in_sprint")
    @patch("jsl.update_story_status")
    @patch("jsl.logging")
    def test_complete_stories_error(
        self, mock_logging, mock_update_story_status, mock_list_stories_in_sprint
    ):
        jira_mock = MagicMock()
        sprint_id = 1001
        mock_list_stories_in_sprint.side_effect = Exception("Connection error")
        ...
        self.assertFalse(result)
        mock_logging.error.assert_called_once_with(
            f"Error completing stories in sprint: Connection error"
        )
...
class TestCompleteSprint(unittest.TestCase):
    @patch("jsl.logging")
    @patch("jsl.JIRA")
    def test_complete_sprint_success(self, MockJIRA, mock_logging):
        jira_mock = MockJIRA()
        sprint_id = 1001
        start_date = "2024-07-15"
        end_date = "2024-07-30"

        # Mock the behavior of jira.sprint and sprint.update
        mock_sprint = MagicMock()
        mock_sprint.name = "Sprint 1"
        mock_sprint.update.return_value = None
        jira_mock.sprint.return_value = mock_sprint
        ...
        self.assertTrue(result)
        mock_logging.info.assert_called_once_with(
            f"Sprint 'Sprint 1' ({sprint_id}) has been completed."
        )


class TestUpdateSprintSummary(unittest.TestCase):
    @patch("jsl.logging")
    @patch("jsl.JIRA")
    def test_update_sprint_summary_success(self, MockJIRA, mock_logging):
        jira_mock = MockJIRA()
        sprint_id = 1001
        new_summary = "Updated Sprint Summary"

        # Mock the behavior of jira.sprint and jira.update_sprint
        mock_sprint = MagicMock()
        mock_sprint.state = "active"  # Mock current state
        jira_mock.sprint.return_value = mock_sprint
        jira_mock.update_sprint.return_value = None  # Mock successful update

        result = update_sprint_summary(jira_mock, sprint_id, new_summary)

        jira_mock.sprint.assert_called_once_with(sprint_id)
        jira_mock.update_sprint.assert_called_once_with(
            sprint_id, name=new_summary, state="active"
        )
        self.assertTrue(result)
        mock_logging.info.assert_called_once_with(
            f"Sprint summary updated successfully. ID: {sprint_id}"
        )


class TestDeleteSprint(unittest.TestCase):
    @patch("jsl.logging")
    @patch("jsl.JIRA")
    def test_delete_sprint_success(self, MockJIRA, mock_logging):
        jira_mock = MockJIRA()
        sprint_id = 1001

        # Mock the behavior of jira.sprint and sprint.delete
        mock_sprint = MagicMock()
        mock_sprint.delete.return_value = None
        jira_mock.sprint.return_value = mock_sprint

        # Mock user confirmation input
        with patch("builtins.input", return_value="y"):
            result = delete_sprint(jira_mock, sprint_id)

        jira_mock.sprint.assert_called_once_with(sprint_id)
        mock_sprint.delete.assert_called_once()
        self.assertTrue(result)
        mock_logging.info.assert_called_once_with(
            f"Sprint with ID {sprint_id} deleted successfully."
        )

    @patch("jsl.logging")
    @patch("jsl.JIRA")
    def test_delete_sprint_cancel(self, MockJIRA, mock_logging):
        jira_mock = MockJIRA()
        sprint_id = 1001

        # Mock the behavior of jira.sprint
        mock_sprint = MagicMock()
        jira_mock.sprint.return_value = mock_sprint
        ...
        jira_mock.sprint.assert_called_once_with(sprint_id)
        mock_sprint.delete.assert_not_called()
        self.assertFalse(result)
        mock_logging.info.assert_called_once_with(
            f"Sprint deletion cancelled by user. ID: {sprint_id}"
        )


class TestDeleteAllSprints(unittest.TestCase):
    @patch("jsl.logging")
    @patch("jsl.JIRA")
    def test_delete_all_sprints_success(self, MockJIRA, mock_logging):
        jira_mock = MockJIRA()
        board_id = 2001

        # Mock the behavior of jira.sprints
        mock_sprint_1 = MagicMock()
        mock_sprint_1.id = 3001
        mock_sprint_2 = MagicMock()
        mock_sprint_2.id = 3002
        jira_mock.sprints.return_value = [mock_sprint_1, mock_sprint_2]
        ...
        jira_mock.sprints.assert_called_once_with(board_id)
        mock_sprint_1.delete.assert_called_once()
        mock_sprint_2.delete.assert_called_once()
        self.assertTrue(result)
        mock_logging.info.assert_any_call("Sprint deleted successfully. ID: 3001")
        mock_logging.info.assert_any_call("Sprint deleted successfully. ID: 3002")
        mock_logging.info.assert_called_with("All sprints have been deleted.")

    @patch("jsl.logging")
    @patch("jsl.JIRA")
    def test_delete_all_sprints_cancel(self, MockJIRA, mock_logging):
        jira_mock = MockJIRA()
        board_id = 2001

        # Mock the behavior of jira.sprints
        mock_sprint = MagicMock()
        jira_mock.sprints.return_value = [mock_sprint]
        ...
        self.assertFalse(result)
        mock_logging.info.assert_called_once_with(
            "Deletion of all sprints cancelled by user. Board ID: 2001"
        )
...
class TestGetBoardId(unittest.TestCase):
    @patch("jsl.logging")
    @patch("jsl.JIRA")
    def test_get_board_id_success(self, MockJIRA, mock_logging):
        jira_mock = MockJIRA()

        # Mock the behavior of jira._session.get
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "values": [{"id": 1001, "name": "Board 1"}, {"id": 1002, "name": "Board 2"}]
        }
        jira_mock._session.get.return_value = mock_response

        # Test with an existing board name
        board_name = "Board 1"
        result = get_board_id(jira_mock, board_name)

        jira_mock._session.get.assert_called_once_with(
            f'{jira_mock._options["server"]}/rest/agile/1.0/board'
        )
    ...
    @patch("jsl.logging")
    @patch("jsl.JIRA")
    def test_get_board_id_not_found(self, MockJIRA, mock_logging):
        jira_mock = MockJIRA()
        ...
        # Test with a non-existing board name
        ...
        self.assertIsNone(result)
        mock_logging.info.assert_called_once_with(f"Board '{board_name}' not found.")

    @patch("jsl.logging")
    @patch("jsl.JIRA")
    def test_get_board_id_error(self, MockJIRA, mock_logging):
        jira_mock = MockJIRA()

        # Simulate an error by raising an exception
        jira_mock._session.get.side_effect = Exception("Connection error")

        # Test the function with an arbitrary board name
        board_name = "Board 1"
        result = get_board_id(jira_mock, board_name)
        ...
        self.assertIsNone(result)
        mock_logging.error.assert_called_once_with(
            "Error retrieving board ID: Connection error"
        )

    @patch("jsl.logging")
    @patch("jsl.JIRA")
    def test_get_board_id_failed_request(self, MockJIRA, mock_logging):
        jira_mock = MockJIRA()
        ...
        self.assertIsNone(result)
        mock_logging.error.assert_called_once_with(
            f"Failed to retrieve boards. Status code: 404"
        )


class TestMyStories(unittest.TestCase):
    @patch("builtins.print")
    @patch("jsl.JIRA")
    def test_my_stories_success(self, MockJIRA, mock_print):
        jira_mock = MockJIRA()

        # Mock the behavior of jira.search_issues
        mock_issue_1 = MagicMock()
        mock_issue_1.key = "ISSUE-1"
        mock_issue_1.fields.summary = "Summary of Issue 1"
        mock_issue_1.fields.status.name = "In Progress"

        mock_issue_2 = MagicMock()
        mock_issue_2.key = "ISSUE-2"
        mock_issue_2.fields.summary = "Summary of Issue 2"
        mock_issue_2.fields.status.name = "To Do"

        jira_mock.search_issues.return_value = [mock_issue_1, mock_issue_2]

        # Test with mock values
        project_key = "PROJECT-1"
        user = "testuser"

        my_stories(jira_mock, project_key, user)
        ...
        mock_print.assert_any_call(
            "Key: ISSUE-1, Summary: Summary of Issue 1, Status: In Progress"
        )
        mock_print.assert_any_call(
            "Key: ISSUE-2, Summary: Summary of Issue 2, Status: To Do"
        )

    @patch("jsl.JIRA")
    def test_my_stories_jira_error(self, MockJIRA):
        jira_mock = MockJIRA()

        # Simulate a JIRAError by raising an exception
        jira_mock.search_issues.side_effect = Exception("Connection error")
    ...
    @patch("jsl.JIRA")
    def test_my_stories_unexpected_error(self, MockJIRA):
        jira_mock = MockJIRA()

        # Simulate an unexpected error by raising an exception
        jira_mock.search_issues.side_effect = ValueError("Unexpected error")
